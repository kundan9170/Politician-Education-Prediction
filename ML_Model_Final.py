...
# Initialize the BernoulliNB Classifier with custom alpha parameter
alpha_value = 0.76  # Adjust the alpha value as needed
bernoulli_nb_model = BernoulliNB(alpha=alpha_value, binarize=0.5, fit_prior=True, class_prior=None)

# Train the model using the training dataset
bernoulli_nb_model.fit(X_train, y_train)

# Make predictions on the test data
test_predictions = bernoulli_nb_model.predict(X_test)
...
# Initialize the BernoulliNB Classifier with custom alpha parameter
bernoulli_nb_model_full = BernoulliNB(alpha=alpha_value, binarize=0.0, fit_prior=True, class_prior=None)

# Train the model using the entire dataset
bernoulli_nb_model_full.fit(X, y)
...
test_predictions = bernoulli_nb_model_full.predict(test_data)
